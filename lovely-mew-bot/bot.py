...
async def image_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    original_path = f"{UPLOAD_FOLDER}{update.message.photo[-1].file_id}_original.jpg"
    compressed_path = f"{UPLOAD_FOLDER}{update.message.photo[-1].file_id}.jpg"
    await file.download_to_drive(original_path)

    # Compress using ffmpeg
    os.system(f"ffmpeg -y -i {original_path} -vf scale=800:-1 -q:v 5 -frames:v 1 {compressed_path}")
    ...
    # Auto Git push using absolute paths
    subprocess.run("""
